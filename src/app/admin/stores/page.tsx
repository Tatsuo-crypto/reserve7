  ...
  return (
    <div className="max-w-6xl mx-auto pt-4 pb-12 sm:px-6 lg:px-8">
      ...
      <div className="flex justify-center mb-6">
        <Button type="button" variant="primary" className="inline-flex items-center px-4 py-2 rounded-lg bg-brand-500 text-white hover:bg-brand-600" onClick={openCreate}>新規店舗</Button>
      </div>
                ...
                <tbody>
                  {stores.map(s => (
                    <tr key={s.id} className="hover:bg-surface-base">
                      ...
                      <td className="px-3 py-2 border-b whitespace-nowrap text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-normal ${s.status === 'active' ? 'bg-brand-500/15 text-brand-600' : 'bg-surface-overlay text-text-secondary'}`}>{s.status === 'active' ? '有効' : '無効'}</span>
                      </td>
                      <td className="px-3 py-2 border-b text-center whitespace-nowrap">
        ...
        <AppModal
          ...
          footer={(
            <>
              ...
              <Button type="button" variant="primary" className="rounded-full bg-brand-500 px-5 py-2 text-sm text-white hover:bg-brand-600" onClick={saveStore}>保存</Button>
            </>
          )}
        >
