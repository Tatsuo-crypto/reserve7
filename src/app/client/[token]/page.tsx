      ...
      <AdminHeader
        ...
        leftElement={
          ...
          <Button
            ...
            aria-label="お知らせ"
            className={`relative h-10 w-10 shrink-0 p-0 rounded-full shadow-sm border border-border-subtle transition-all active:scale-95 ${activeTab === 'notifications' ? 'bg-brand-500/15 text-brand-600' : 'bg-surface-raised text-text-secondary'}`}
          >
        ...
        rightElement={
          <div className="flex min-w-0 items-center justify-end gap-1.5">
            <Button
              ...
              aria-label="設定"
              className={`h-10 w-10 shrink-0 p-0 rounded-full shadow-sm border border-border-subtle transition-all active:scale-95 ${activeTab === 'settings' ? 'bg-brand-500/15 text-brand-600' : 'bg-surface-raised text-text-secondary'}`}
            >
...
function CenterHomeBtn({ active, onClick }: { active: boolean, onClick: () => void }) {
  return (
    <div className="relative flex h-full flex-col items-center justify-center">
      ...
      <span className={`absolute bottom-2 text-xs font-normal transition-colors duration-300 ${active ? 'text-brand-600' : 'text-text-muted'}`}>ホーム</span>
    </div>
  )
}
...
function DataRequestSection({
  ...
  return (
    <div className="rounded-2xl border border-border-subtle bg-surface-raised p-4">
      ...
      {submittedType ? (
        <p className="mt-3 text-xs font-normal text-brand-600">
