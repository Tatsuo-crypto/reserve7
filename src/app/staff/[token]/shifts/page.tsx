          ...
          <div className="mt-2 grid grid-cols-7 gap-1">
            {calendarDays.map((day, index) => {
              ...
              return (
                ...
                >
                  <div className={`text-xs tabular-nums ${selected ? 'text-brand-600' : 'text-text-secondary'}`}>{day.getDate()}</div>
            ...
            <div className="space-y-2">
              {shifts.length > 0 ? shifts.map(shift => (
                <div key={shift.id} className="flex items-center justify-between rounded-2xl bg-surface-base px-4 py-3">
                  ...
                  <span className="rounded-full bg-brand-500/15 px-3 py-1 text-xs text-brand-600">{formatHours(shift.start_time, shift.end_time)}</span>
                </div>
              )) : (
