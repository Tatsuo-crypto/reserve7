...
const BottomNavigation = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-100 pb-safe">
      <div className="max-w-md mx-auto flex items-end justify-around px-2 py-1 relative">
        {navItems.map((item) => {
          // 遷移先URLの決定
          let href = `/dashboard?tab=${item.id}`;
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center transition-all duration-300 relative ${
                item.isCenter ? 'mb-1' : 'pb-1 flex-1'
              }`}
            >
              <div
                className={`flex items-center justify-center transition-all duration-300 ${
                  item.isCenter
                    ? 'w-14 h-14 bg-blue-600 text-white rounded-full shadow-lg -translate-y-4 border-4 border-white'
                    : isActive
              ...
              {item.isCenter && (
                <span className={`text-[10px] font-bold absolute bottom-0 transition-colors ${
                  isActive ? 'text-blue-600' : 'text-gray-400'
                }`}>
                  {item.label}
                </span>
