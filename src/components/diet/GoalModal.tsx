        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black bg-opacity-50">
            <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-slideUp">
                <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                    <h2 className="text-xl font-black text-gray-900">目標設定</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    {/* New Goal Form */}
                    <section>
                        <h3 className="text-xs font-black text-blue-600 uppercase tracking-widest mb-4">新しい目標を設定</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-400 mb-1">開始日</label>
                                <input
                                    type="date"
                                    value={form.startDate}
                                    onChange={e => setForm({ ...form, startDate: e.target.value })}
                                    className="w-full bg-gray-50 border-none rounded-xl font-bold text-gray-700 px-4 py-3"
                                />
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className="block text-xs font-bold text-gray-400 mb-1">目標カロリー (kcal)</label>
                                    <input
                                        type="number"
                                        value={form.calories}
                                        onChange={e => setForm({ ...form, calories: parseInt(e.target.value) || 0 })}
                                        className="w-full bg-gray-50 border-none rounded-xl font-bold text-gray-700 px-4 py-3 text-xl"
                                    />
                                </div>
                            ...
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-200 active:scale-95 transition-all mt-4"
                            >
                                {saving ? '保存中...' : '目標を更新する'}
                            </button>
                        </div>
                    ...
                    {/* Goal History */}
                    <section>
                        <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">目標の履歴</h3>
                        {loading ? (
                            ...
                            <div className="space-y-3">
                                {history.map((h, i) => (
                                    <div key={h.id} className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
                                        <div>
                                            <div className="text-xs font-bold text-gray-400">{h.start_date.replace(/-/g, '/')} 〜</div>
                                            <div className="text-lg font-black text-gray-700">{h.calories} <span className="text-xs">kcal</span></div>
                                        </div>
                                        <div className="text-[10px] font-bold text-gray-400 bg-white px-3 py-1 rounded-full shadow-sm">
